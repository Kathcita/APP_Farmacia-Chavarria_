...
{
    public partial class ReporteVentasCategoriasViewModel: ObservableObject
    {
        private FacturaService _facturaService;
        private UsuarioService _usuarioService;

        public ReporteVentasCategoriasViewModel(FacturaService facturaService, UsuarioService usuarioService)
        {
        ...
        [ObservableProperty]
        private List<CategoriaVentasDTO> ventasCategoria= new();
        ...
        public byte[] GenerarExcelYDescargar(List<CategoriaVentasDTO> data, string nombreArchivo = "ReporteDeVentasPorCategoria")
        {
            if (data.Count == 0)
            {
            ...
            worksheet.Cell(2, 1).Value = "Categoría";
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                worksheet.Cell(row, 1).Value = data[i].NombreCategoria;
        ...
        public byte[] GenerarPdf(List<CategoriaVentasDTO> data, byte[] imagenGrafico = null, string nombreArchivo = "ReporteDeVentasPorCategoria")
        {
            try
            {
                if (data.Count == 0)
                ...
                var document = Document.Create(container =>
                {
                    container.Page(page =>
                    {
                        ...
                        page.Content().Column(col =>
                        {
                            ...
                            col.Item().Table(table =>
                            {
                                ...
                                {
                                    table.Cell().Text($"{item.NombreCategoria}");
                    ...
                    if (imagenGrafico != null)
                    {
                        container.Page(page =>
                        {
                            ...
                            page.Content().Column(col =>
                            {
                                col.Item().AlignCenter().Text("Gráfico de Ventas por Categoría")
